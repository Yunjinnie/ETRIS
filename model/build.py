...
from model.med import BertModel, BertConfig
from model.tokenization_bert import BertTokenizer
from torch import nn
from functools import partial
# from model.vit import interpolate_pos_embed
from model.dino_vit import VisionTransformer,interpolate_pos_embed
# from model.deit_v2 import vit_models, Layer_scale_init_Block
from model.med import BertLayer
import torch
import pdb
...
def text_encoder(config: DictConfig, text_encoder: str, adapter_append: bool):
    if config.pretrained_text:
        try:
            ...
            # )
            bert_config = BertConfig.from_pretrained(config.text_encoder)
            ...
            # )
            return BertModel.from_pretrained(config.text_encoder, config=bert_config,add_pooling_layer=False)
    else:
        try:
...
def vision_encoder(config, vision_encoder: str, adapter_append: bool):
    ...
    print(config)
    adapter_config = config.conventional_adapter
    ...
    #         conventional_adapter_kwargs=conventional_adapter_kwargs,
    #     )
    if vision_encoder == "base_dino":
        model = dino_base_patch16(
            pretrained=pretrained, image_res=config.input_size, depth=depth
        )
    else:
