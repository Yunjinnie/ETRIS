...
from model.conventional_adapter import Adapter
...
class Attention(nn.Module):
    ...
    def forward(self, x,attention_mask=None):
        ...
        if attention_mask is not None:
            # print(attn.size()) #([8, 8, 921, 921])
            # print(attention_mask.size()) #([8, 20])
...
class Block(nn.Module):
    ...
    def forward(self, x, return_attention=False, attention_mask=None):
        if attention_mask is not None:
            y, attn = self.attn(self.norm1(self.adapter1(x)),attention_mask=attention_mask)
        else:
            y, attn = self.attn(self.norm1(self.adapter1(x)))
        if return_attention:
            return attn
